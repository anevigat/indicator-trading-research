...
def compute_backtest_metrics(trades: pd.DataFrame, equity_curve: pd.DataFrame, initial_capital: float) -> dict[str, float | int | None]:
    ...
    return {
        ...
        "chandelier_trailing_exits": int((trades["exit_reason"] == "chandelier_trailing_stop").sum()) if "exit_reason" in trades.columns else 0,
        "break_even_exits": int((trades["exit_reason"] == "break_even").sum()) if "exit_reason" in trades.columns else 0,
        "ma_stop_exits": int((trades["exit_reason"] == "ma_stop").sum()) if "exit_reason" in trades.columns else 0,
